...
import base64
import bcrypt
from encryption import load_or_create_key, encrypt_data, decrypt_data
...
def register_user(username, password, recovery_code, bank_balance, role):
  ...
  encrypted_bank_balance = encrypt_data(
      str(bank_balance), key, salt)  # Convert to string if not already
  encrypted_role = encrypt_data(role, key, salt)

  updated = False
  new_row = [
      username,
      hashed_password.decode('utf-8'), encrypted_recovery_code,
      encrypted_bank_balance, encrypted_role,
  ...
  with open('user_credentials.csv', mode, newline='') as csvfile:
    ...
    if not rows:  # If no rows exist, it's a new file, write header
      writer.writerow([
          'Username', 'Password', 'Recovery code', 'Bank balance', 'Role',
          'Key'
      ])
    if updated:
    ...
    for row in reader:
      if row['Username'] == search_username:
        ...
        user_info = {
            ...
            'Recovery code':
            decrypt_data(row['Recovery code'], decryption_key, salt),
            'Bank balance':
            decrypt_data(row['Bank balance'], decryption_key, salt),
            'Role':
            decrypt_data(row['Role'], decryption_key, salt)
        }
        break
    ...
    with open('user_credentials.csv', newline='') as csvfile:
      ...
      for row in reader:
        if username == row['Username']:
          hashed_password = row['Password'].encode('utf-8') #eib
          if bcrypt.checkpw(password.encode('utf-8'), hashed_password):
            key = base64.b64decode(row['Key'])
            salt = base64.b64decode(row['Salt'])

            decrypted_recovery_code = decrypt_data(row['Recovery code'], key,
                                                   salt)
            if recovery_code == decrypted_recovery_code:
              decrypted_bank_balance = decrypt_data(row['Bank balance'], key,
                                                    salt)
...
def get_transaction_history(username):
  ...
  try:
    with open('app.log', 'r') as file:
      for line in file:
        if f"User '{username}'" in line and ("Deposit" in line
                                             or "Withdrawal" in line):
          transactions.append(line.strip())
  except FileNotFoundError:
    transactions.append("Transaction log file not found.")
  except Exception as e:
    transactions.append(
        f"An error occurred while processing the log file: {str(e)}")
  return transactions
...
def update_bank_balance(username, new_balance):
  try:
    with open('user_credentials.csv', 'r', newline='') as csvfile:
      reader = csv.DictReader(csvfile) #conv to dict
      rows = list(reader)
...
def login():
  ...
  if form.validate_on_submit():
    ...
    authenticated_user, bank_balance, role = authenticate_user(
    ...
    if authenticated_user:
      ...
      if session['role'] == 'admin':
        app.logger.info(
            f"Successful login: Admin '{username}' from IP {request.remote_addr}"
        )
      ...
      app.logger.warning(
          f"Failed login attempt: User '{username}' from IP {request.remote_addr}"
      )
      flash(
          "Login failed. Please check your username, password, and recovery code."
      )
    ...
    if request.method == 'POST': #check if form is submitted
      ...
      if 'create-create_user' in request.form:
        ...
        if create_user_form.validate_on_submit():
          ...
          register_user(new_username, new_password, new_recovery_code, "0",
                        new_role)
...
def dashboard():
  ...
  return render_template('dashboard.html',
                         ...
                         bank_balance=session.get('bank_balance', 0),
...
def deposit():
  if 'username' in session:
    ...
    if deposit_amount > 0:
      session['bank_balance'] = round(
          float(session.get('bank_balance', 0)) + deposit_amount, 2)
      update_bank_balance(session['username'], session['bank_balance'])
      app.logger.info(
          f"Deposit: User '{session['username']}' deposited ${deposit_amount}. New balance: ${session['bank_balance']}"
      )
    return redirect(url_for('dashboard'))
  else:
    return redirect(url_for('home'))
...
def withdraw():
  if 'username' in session:
    ...
    if withdraw_amount > 0 and bank_balance >= withdraw_amount:
      ...
      update_bank_balance(session['username'], session['bank_balance'])
      app.logger.info(
          f"Withdrawal: User '{session['username']}' withdrew ${withdraw_amount}. New balance: ${session['bank_balance']}"
      )
...
def transaction_history():
  if 'username' in session:
    ...
    if transactions and "error occurred" not in transactions[0].lower():
      ...
      flash(
          transactions[0])  # Display error message from the transactions list
...
def user_log_history(username):
  ...
  return render_template('user_log_history.html',
...
def user_info(username):
  ...
  try:
    ...
    app.logger.error(
        f"Failed to retrieve user information for {username}: {e}")
...
class CreateUserForm(FlaskForm):
  ...
  new_password = PasswordField('Password',
                               validators=[DataRequired(),
                                           Length(min=6)])
  new_recovery_code = StringField(
      'Recovery Code', validators=[DataRequired(),
                                   Length(min=3, max=50)])
  new_role = StringField('Role',
                         validators=[DataRequired(),
                                     Length(min=4, max=5)])


class LoginForm(FlaskForm):
  username = StringField('Username', validators=[DataRequired()]) #stringfield - flask wtf
  one_time_password = PasswordField('One-Time Password',
                                    validators=[DataRequired()])
...
class BalanceForm(FlaskForm):
  ...
  withdraw_amount = DecimalField(
      'Withdraw Amount', validators=[DataRequired(),
                                     NumberRange(min=0.01)])


class DeleteUserForm(FlaskForm):
  username_to_delete = StringField(
      'Username to Delete', validators=[DataRequired(),
                                        Length(min=3, max=25)])
